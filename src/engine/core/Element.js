...
(function () {
    ...
    function setDomReference(el, dom) {
        el._domElement = dom;
        for (var key in el._children) {
            var c = el._children[key];
            for (var j = 0; j < c.length; j++) {
                setDomReference(c[j], dom);
            }
        }
    }
    ...
    Element.prototype.add = function (element) {
        if (element instanceof Element) {
            ...
            setDomReference(element, this._domElement);
        }
    };
